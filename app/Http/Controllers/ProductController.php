...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('images')) {
            foreach ($request->file('images') as $file) {
                $uploaded    = Cloudinary::upload($file->getRealPath(), ['folder' => 'products']);
                $imageUrls[] = $uploaded->getSecurePath();
            }
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->has('features')) {
            $product->features = $this->parseFeatures($request->features);
        }

        // Start with old images the admin wants to keep
    ...
    public function destroy($id)
    {
        ...
        foreach ($images as $imgUrl) {
            ...
            // URL format: https://res.cloudinary.com/<cloud>/image/upload/v123456/products/filename.webp
            preg_match('/upload\/(?:v\d+\/)?(.+)\.\w+$/', $imgUrl, $matches);
            if (!empty($matches[1])) {
    ...
    // Safely parse features input (array or JSON string)
    private function parseFeatures($features)
    {
        if (is_array($features))  return $features;
        if (is_string($features)) return json_decode($features, true) ?? [];
